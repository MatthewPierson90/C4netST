# -*- coding: utf-8 -*-
"""
Created on Wed Jul 21 15:44:59 2021
...
if __name__=='__main__':
    product_for_ar = my_product(7)

@njit(nogil=True) 
def make_n_array(n,three_mult=1.5):
    # print(' in make n array')
    to = 3**n
    ar = np.zeros((2,to))
    count = 0
    for n1 in range(to):
...
def minimax(board, 
    ...
    for k in range(len(moves)):
        if moves[k]==0:
            ...
            #print('\n----------------------------------\nlayer:',depth,'\nk:',k,'\n---------------------------------------------')
            #print('Move:',k+1,'hyp value',hypothetical_value)
        ...
        if best_move == 8:
            # print('in am')
            # print(np.where(available_moves(board)==1))
...
@njit(nogil=True, cache=True)
def two_ai_game_with_oview(eval_lst,
                           x_lvl,
                           o_lvl,
                           prune,
                           len_4_vals,
...
# changed from 14 to only return x-data for both players.  Takes longer since 3 minimax
...
if __name__=='__main__':
    toc = tt()
    two_ai_game(eval_lst=(1.5,1,1.5,1,2,1),
                x_lvl=4,
                o_lvl=4,
                prune=True,
    ...
    two_ai_game(eval_lst=(1.5,1,1.5,1,2,1),
                ...
                len_7_vals=len_7_vals)
    tic = tt()
    print('Game 2 time:',tic - toc)
    toc = tt()
    two_ai_game_with_oview(eval_lst=(1.5,1,1.5,1,2,1),
                           x_lvl=4,
                           o_lvl=4,
                           prune=True,
                           len_4_vals=len_4_vals,
...
# def multi_play_games(proc=cpu_count()):
#     # N=1
#     # eval_lst = [(1.,1.,1.,1.,1.,1.)]
#     # lst=[np.zeros((7,8))]
#     # recip = 1/N
...
#     #                                               1.+n5*recip,
#     #                                               1.+n6*recip))
#     input_lst = [(1.,2.,1.,2.,1.,2.),(1.,2.,1.,2.,1.,2.)]
#     print('starting compile game')
#     tic = time.time()
#     two_ai_game_fixed_lvl(*(1.,2.,1.,2.,1.,2.))
#     toc = time.time()
...
#     two_ai_game_fixed_lvl(*(1.,2.,1.,2.,1.,2.))
#     toc = time.time()
#     print('after compile time:',toc-tic)
#     print('startng multi')
#     tic = time.time()
#     with Pool(proc) as p:
#         data = p.starmap(two_ai_game_fixed_lvl,input_lst)
#     p.close()
#     toc =time.time()
#     print('multi proc time:',toc-tic)
#     return(data)
...
def into_mini(board,player,result):
    ...
    array = minimax(board,
                    ...
                    -1000, #alpha 
                    1000) #beta
    # to_return_board = board_to_ar(board)
    to_return = np.zeros((7,8))
    to_return[:,:6]=board
    to_return[:,7]=array[4:]
    to_return[:,6]=np.ones(7)*result
    # to_return_result = lst[2]
    # to_return_score=array[5:].reshape(7,1)
    return to_return
...
if __name__=='__main__':
    pass
    # toc1 = tt()
    # data = many_games(2,4, 1,len_4_vals,len_5_vals,len_6_vals,len_7_vals)
    # tic1 = tt()
    # print('First Many Time:', tic1-toc1)
    # toc1 = tt()
    # data = many_games(2,4, 1,len_4_vals,len_5_vals,len_6_vals,len_7_vals)
    # tic1 = tt()
    # print('Second Mini Time:', tic1-toc1)
    #gameData = two_ai_game()
    #nnetGameData=results_cleaned(gameData)
    #print_data_lst(gameData)
    # print('start time:',dt.datetime.now())
    # tic_total = tt
    # tic = tt
    # data1 = many_games_var_depth(2,3, plus=0)
    # toc = tt
    # save_file(data1,'depthvar3level2plus0.pkl')
    # print('level 2 plus 0, depth 3 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # tic = tt
    # data1 = many_games_var_depth(2,4, plus=0)
    # toc = tt
    # save_file(data1,'depthvar4level2plus0.pkl')
    # print('level 2 plus 0, depth 4 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # tic = tt
    # data1 = many_games_var_depth(1,3, plus=1)
    # toc = tt
    # save_file(data1,'depthvar3level1plus1.pkl')
    # print('level 1 plus 1, depth 3 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # tic = tt
    # data1 = many_games_var_depth(1,4, plus=1)
    # toc = tt
    # save_file(data1,'depthvar4level1plus1.pkl')
    # print('level 1, depth 4 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # tic = tt
    # data1 = many_games_var_depth(1,5, plus=1)
    # toc = tt
    # save_file(data1,'depthvar5level1plus1.pkl')
    # print('level 1 plus 1, depth 5 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # tic = tt
    # data1 = many_games_var_depth(2,5, plus=0)
    # toc = tt
    # save_file(data1,'depthvar5level2plus0.pkl')
    # print('level 2 plus 0, depth 5 complete.')
    # print('time:',toc-tic)
    # print('\n--------------------------------\n')
    # toc_total = tt
    # print('total time:',toc_total-tic_total)
